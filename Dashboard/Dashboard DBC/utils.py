...
# return grade distribution table for grades profile page
def grades_distribution_groupby(dataset_):
    groupby_form = dataset_.groupby(["Succ", "GradeGrp"])['PIDM'].count().reset_index()
    groupby_form['percent'] = groupby_form['PIDM']/sum(groupby_form['PIDM'])
    # split and operate append
    # successful
    groupby_form_one = groupby_form[groupby_form['Succ'] == "Yes"]
    rows_list = ["Total Success/Rate", "", sum(groupby_form_one['PIDM']), sum(groupby_form_one['percent'])]
    groupby_form_one['Succ'] = ""
    groupby_form_one.loc[len(groupby_form_one)] = rows_list
    # non-successful
    groupby_form_zero = groupby_form[groupby_form['Succ'] == "No"]
    rows_list = ["Total Non-success/Rate", "", sum(groupby_form_zero['PIDM']), sum(groupby_form_zero['percent'])]
    groupby_form_zero['Succ'] = ""
    groupby_form_zero.loc[len(groupby_form_zero)] = rows_list
    # reshape
    groupby_result = groupby_form_one.append(groupby_form_zero)
    rows_list = ["Total", "", sum(groupby_result['PIDM'])/2, sum(groupby_result['percent'])/2]
    groupby_result.loc[len(groupby_result)] = rows_list
    groupby_result['percent'] = pd.Series(["{0:.1f}%".format(val * 100) for val in groupby_result['percent']],
                                          index = groupby_result.index)
    # rename columns
    groupby_result = groupby_result.rename(columns={'PIDM': 'Grades', 'percent': '%ofGrades'})
    return groupby_result

def pie_chart_percent_combine(label_list, value_list, threshold = 0.02):
    result_label_list = []
    result_value_list = []
    tmp_label = ""
    tmp_value = 0
    for i in range(len(value_list)):
        # combine
        if value_list[i] < threshold:
            tmp_value = tmp_value + value_list[i]
            tmp_label = tmp_label + label_list[i] + "/"
        else:
            result_label_list.append(label_list[i])
            result_value_list.append(value_list[i])
    # do accumulate
    if tmp_value > 0:
        result_label_list.append(tmp_label[:-1])
        result_value_list.append(tmp_value)
    return result_label_list, result_value_list
